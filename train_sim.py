#!/usr/bin/env python3
# Fine-tunes yolo11n on the SIM 3-class dataset (aruco_marker, cup_holder,
# hole) — separate from train.py (real D415 captures). Sim and real images
# look different enough (synthetic Gazebo rendering vs. real optics/
# lighting) to warrant two independent models rather than one mixed
# dataset — see dataset_sim/data.yaml and README.md.
#
...
# from COCO, so it needs far fewer of our images to learn our 3 classes.
model = YOLO("yolo11n.pt")

model.train(
    data="dataset_sim/data.yaml",  # sim dataset, not the real one
    ...
    name="aruco_cupholder_sim",    # this run's subfolder: runs/detect/runs/aruco_cupholder_sim*/
)